...
static llvm::Expected<DirEntryRef>
getDirectoryFromFile(FileMgr &FileMgr, StringRef Filename, bool CacheFailure) {
  if (Filename.empty())
...
void FileMgr::addAncestorsAsVirtualDirs(StringRef Path) {
  ...
  auto UDE = std::make_unique<DirEntry>();
...
llvm::Expected<DirEntryRef> FileMgr::getDirectoryRef(StringRef DirName,
                                                     bool CacheFailure) {
  ...
  if (!SeenDirInsertResult.second) {
    if (SeenDirInsertResult.first->second)
      return DirEntryRef(*SeenDirInsertResult.first);
  ...
  // Windows).
  DirEntry &UDE = UniqueRealDirs[Status.getUniqueID()];
  ...
  return DirEntryRef(NamedDirEnt);
}

llvm::ErrorOr<const DirEntry *> FileMgr::getDirectory(StringRef DirName,
                                                      bool CacheFailure) {
...
FileMgr::getFile(StringRef Filename, bool openFile, bool CacheFailure) {
...
FileMgr::getFileRef(StringRef Filename, bool openFile, bool CacheFailure) {
  ++NumFileLookups;
  ...
  DirEntryRef DirInfo = *DirInfoOrErr;
...
llvm::Expected<FileEntryRef> FileMgr::getSTDIN() {
...
const FileEntry *FileMgr::getVirtualFile(StringRef Filename, off_t Size,
                                         time_t ModificationTime) {
  return &getVirtualFileRef(Filename, Size, ModificationTime).getFileEntry();
}

FileEntryRef FileMgr::getVirtualFileRef(StringRef Filename, off_t Size,
                                        time_t ModificationTime) {
  ++NumFileLookups;
  ...
  // following call is guaranteed to find the DirEntry from the
  // cache.
...
llvm::ErrorOr<std::unique_ptr<llvm::MemoryBuffer>>
FileMgr::getBufferForFileImpl(StringRef Filename, int64_t FileSize,
...
std::error_code FileMgr::getStatValue(StringRef Path, llvm::vfs::Status &Status,
                                      bool isFile,
                                      std::unique_ptr<llvm::vfs::File> *F) {
...
std::error_code FileMgr::getNoncachedStatValue(StringRef Path,
...
StringRef FileMgr::getCanonicalName(const DirEntry *Dir) {
...
StringRef FileMgr::getCanonicalName(const FileEntry *File) {
  llvm::DenseMap<const void *, llvm::StringRef>::iterator Known =
