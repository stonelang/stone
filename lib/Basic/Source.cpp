...
bool SrcMgr::openVirtualFile(SrcLoc loc, StringRef name,
                                    int lineOffset) {
...
void SrcMgr::closeVirtualFile(SrcLoc end) {
  ...
  virtualFile->Range = CharSrcRange(*this, virtualFile->Range.getStart(),
                                       end);
...
const SrcMgr::VirtualFile *
SrcMgr::getVirtualFile(SrcLoc Loc) const {
...
unsigned SrcMgr::getLocOffsetInBuffer(SrcLoc Loc,
...
void SrcLoc::dump(const SrcMgr &SM) const {
  print(llvm::errs(), SM);
}

void SrcRange::print(raw_ostream &OS, const SrcMgr &SM,
...
void SrcRange::dump(const SrcMgr &SM) const {
  print(llvm::errs(), SM);
}

CharSrcRange::CharSrcRange(const SrcMgr &SM, SrcLoc Start,
                                 SrcLoc End)
    : Start(Start) {
...
void CharSrcRange::dump(const SrcMgr &SM) const {
  print(llvm::errs(), SM);
}

llvm::Optional<unsigned>
SrcMgr::resolveOffsetForEndOfLine(unsigned BufferId,
                                         unsigned Line) const {
  return resolveFromLineCol(BufferId, Line, ~0u);
}

llvm::Optional<unsigned>
SrcMgr::getLineLength(unsigned BufferId, unsigned Line) const {
...
SrcLoc
SrcMgr::getLocFromExternalSource(StringRef Path, unsigned Line,
                                        unsigned Col) {
