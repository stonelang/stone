...
class alignas(1 << TypeRepAlignInBits) TypeLoc {
  ...
  //   // FIXME: We generally shouldn't need to build TypeLocs without a
  //   location. static TypeLoc WithoutLoc(Type ty) {
  //     TypeLoc result;
  //     result.ty = ty;
  //     return result;
  //   }
  ...
  //     return llvm::hash_combine(owner.ty.GetPointer(), owner.tyRep);
  //   }

  //   friend bool operator==(const TypeLoc &lhs, const TypeLoc &rhs) {
  //     return lhs.ty.GetPointer() == rhs.ty.GetPointer() && lhs.tyRep ==
  //     rhs.tyRep;
  //   }
