...
//
// See https://stone.org/LICENSE.txt for license information
// See https://stone.org/CONTRIBUTORS.txt for the list of Swift project authors
//
//===----------------------------------------------------------------------===//
//
/// \file
...
///        return seq.size();
///      }
///      static MyType& element(Output &, std::vector<MyType> &seq, size_t
///      index) {
///        if (index >= seq.size())
///          seq.resize(index+1);
///        return seq[index];
...
template <class T> struct has_ScalarReferenceTraits {
...
template <class T> struct has_ObjectTraits {
...
template <class T> struct has_ObjectValidateTraits {
...
template <class T> struct has_ArrayMethodTraits {
...
// Test if ArrayTraits<T> is defined on type T
template <typename T>
struct has_ArrayTraits
    : public std::integral_constant<bool, has_ArrayMethodTraits<T>::value> {};

// Test if NullableTraits<T> is defined on type T.
template <class T> struct has_NullableTraits {
...
public:
  static bool const value = (sizeof(test<NullableTraits<T>>(nullptr)) == 1);
};
...
inline bool isNull(StringRef S) { return S.equals("null"); }
...
struct missingTraits
    : public std::integral_constant<
          bool, !has_ScalarEnumerationTraits<T>::value &&
                    !has_ScalarBitSetTraits<T>::value &&
                    !has_ScalarTraits<T>::value &&
                    !has_ScalarReferenceTraits<T>::value &&
                    !has_NullableTraits<T>::value &&
                    !has_ObjectTraits<T>::value && !has_ArrayTraits<T>::value> {
};

template <typename T>
struct validatedObjectTraits
    : public std::integral_constant<bool,
                                    has_ObjectTraits<T>::value &&
                                        has_ObjectValidateTraits<T>::value> {};

template <typename T>
struct unvalidatedObjectTraits
    : public std::integral_constant<bool,
                                    has_ObjectTraits<T>::value &&
                                        !has_ObjectValidateTraits<T>::value> {};
...
public:
  ...
  UserInfoMap &getUserInfo() { return UserInfo; }
...
private:
  ...
  template <typename T> void processKey(StringRef Key, T &Val, bool Required) {
    void *SaveInfo;
    bool UseDefault;
...
template <> struct ScalarReferenceTraits<bool> {
  static StringRef stringRef(const bool &);
  static bool mustQuote(StringRef) { return false; }
};

template <> struct ScalarReferenceTraits<StringRef> {
  static StringRef stringRef(const StringRef &);
  static bool mustQuote(StringRef S) { return true; }
};

template <> struct ScalarReferenceTraits<std::string> {
...
// and it is used to define stone::sys::ProcessId.
template <> struct ScalarTraits<unsigned long> {
...
template <typename T>
inline typename std::enable_if<stone::json::has_ObjectTraits<T>::value,
                               Output &>::type
...
template <typename T>
inline typename std::enable_if<stone::json::has_ArrayTraits<T>::value,
                               Output &>::type
...
#endif // STONE_BASIC_JSONSERIALIZATION_H
