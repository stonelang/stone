...
namespace stone {
class Driver;
void *AllocateInDriver(size_t bytes, const stone::Driver &driver,
                       unsigned alignment);
...
template <typename AlignTy> class DriverAllocation {
public:
  ...
  void *operator new(size_t bytes, const stone::Driver &driver,
                     unsigned alignment = alignof(AlignTy)) {
    return stone::AllocateInDriver(bytes, driver, alignment);
  }
