...
static std::string makeFileName(llvm::StringRef Prefix,
...
static std::string makeStatsFileName(llvm::StringRef ProgramName,
                                     llvm::StringRef AuxName) {
  return makeFileName("stats", ProgramName, AuxName, "json");
}

static std::string makeTraceFileName(llvm::StringRef ProgramName,
                                     llvm::StringRef AuxName) {
  return makeFileName("trace", ProgramName, AuxName, "csv");
}

static std::string makeProfileDirName(llvm::StringRef ProgramName,
                                      llvm::StringRef AuxName) {
...
// and slashes.
static std::string cleanName(llvm::StringRef n) {
  std::string tmp;
  for (auto c : n) {
    if (('a' <= c && c <= 'z') || ('A' <= c && c <= 'Z') ||
        ('0' <= c && c <= '9') || (c == '.'))
...
static std::string auxName(llvm::StringRef ModuleName,
                           llvm::StringRef InputName,
                           llvm::StringRef TripleName,
                           llvm::StringRef OutputType,
                           llvm::StringRef OptType) {
  if (InputName.empty()) {
    InputName = "all";
  }
...
public:
  void beginTimer(llvm::StringRef Name) {
  ...
  void endTimer(llvm::StringRef Name) {
...
public:
  ...
  void printToFile(llvm::StringRef Dirname, llvm::StringRef Filename) const {
...
StatsReporter::StatsReporter(llvm::StringRef ProgramName,
    ...
    : currentProcessExitStatusSet(false),
      ...
      Timer(std::make_unique<NamedRegionTimer>(AuxName, "Building Target",
...
void updateProcessWideFrontendCounters(
  ...
  C.MaxMallocUsage =
      std::max(C.MaxMallocUsage, (int64_t)llvm::sys::Process::GetMallocUsage());
#endif
}
...
void StatsReporter::saveAnyFrontendStatsEvents(FrontendStatsTracer const &T,
  ...
  if (EventProfilers) {
    ...
    EventProfilers->UserTime.profileEvent(T.EventName, TimeDelta.getUserTime(),
                                          IsEntry);
    EventProfilers->SystemTime.profileEvent(T.EventName,
                                            TimeDelta.getSystemTime(), IsEntry);
    EventProfilers->ProcessTime.profileEvent(
        T.EventName, TimeDelta.getProcessTime(), IsEntry);
    EventProfilers->WallTime.profileEvent(T.EventName, TimeDelta.getWallTime(),
                                          IsEntry);
...
#undef FRONTEND_STATISTIC
  ...
  if (EntityProfilers) {
    ...
    EntityProfilers->UserTime.profileEvent(T.EventName, TimeDelta.getUserTime(),
                                           IsEntry, T.Entity, T.Formatter);
    EntityProfilers->SystemTime.profileEvent(
        T.EventName, TimeDelta.getSystemTime(), IsEntry, T.Entity, T.Formatter);
    EntityProfilers->ProcessTime.profileEvent(T.EventName,
                                              TimeDelta.getProcessTime(),
                                              IsEntry, T.Entity, T.Formatter);
    EntityProfilers->WallTime.profileEvent(T.EventName, TimeDelta.getWallTime(),
                                           IsEntry, T.Entity, T.Formatter);
#define FRONTEND_STATISTIC(TY, N)                                              \
...
StatsReporter::~StatsReporter() {
  ...
  if (FrontendCounters) {
    ...
    if (C.NumSourceLines != 0 && ElapsedTime.getProcessTime() != 0.0)
      C.NumSourceLinesPerSecond =
          (int64_t)(((double)C.NumSourceLines) / ElapsedTime.getProcessTime());
  }

  std::error_code EC;
  raw_fd_ostream ostream(StatsFilename, EC, fs::OF_Append | fs::OF_Text);
  if (EC) {
    llvm::errs() << "Error opening -stats-output-dir file '" << StatsFilename
                 << "' for writing\n";
    return;
  }
  ...
  if (FrontendStatsEvents && SourceMgr) {
    ...
    for (auto const &E : *FrontendStatsEvents) {
      tstream << E.TimeUSec << ',' << E.LiveUSec << ','
              << (E.IsEntry ? "\"entry\"," : "\"exit\",") << '"' << E.EventName
              << '"' << ',' << '"' << E.CounterName << '"' << ','
